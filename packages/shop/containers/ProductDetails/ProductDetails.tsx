...
import { findProductIndex, getAddonQuantity, getProductQuantity } from 'helper/utility';
import { AddOn, Product } from 'interfaces';
import { FormattedMessage } from 'react-intl';
import LanguageContext from 'contexts/language/language.context';
import { AddOnItem, StyledInputIncDec } from 'containers/QuickView/QuickView.style';
...
const ProductDetails: React.FunctionComponent<ProdutDetailsProps> = ({
  ...
  const { add, update, products, updateAddOn   } = useContext(CartContext);
  const data = product;
  const index = findProductIndex(products, data.id);
  const quantity = getProductQuantity(products, index);
  const [productQuantity, setQuantity] = useState(getProductQuantity(products, index))
  ...
  const handleUpdateAddOn = (value: number, addon: AddOn) => {
    updateAddOn(index, addon, value)
  }
    ...
    <>
      <ProductDetailsWrapper className='product-card' dir='ltr'>
        ...
        <ProductInfo dir={lang === 'ar' || lang === 'he' ? 'rtl' : 'ltr'}>
          ...
          {productQuantity > 0 && <ProductCartWrapper>
            
            {product.addons && product.addons.map((addOn: AddOn) =>
              (
                <AddOnItem
                  key={addOn.id}
                >
                  ...
                  <div>
                    <StyledInputIncDec
                      value={getAddonQuantity(products[index], addOn.id)}
                      onUpdate={(value: number, e: any) =>
                        handleUpdateAddOn(value, addOn)
                      }
                    />
                    <span className='product-price'>{addOn.price || 0}$</span>
                  </div>
                </AddOnItem>
              )                  
            )}
